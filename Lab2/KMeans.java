...
public class KMeans {
	...
	public static void writeCentroids(double[][] points, int index, String dir, String[] classes) {
		try {
			...
			Path centroidFile = new Path(dir + "/centroid" + Integer.toString(index));
			OutputStreamWriter outStreamWriter = new OutputStreamWriter(hdfs.create(centroidFile));
			for (int i = 0; i < points.length; i++) {
				for (int j = 0; j < points[i].length; j++) {
					outStreamWriter.append(Double.toString(points[i][j]) + ",");
				}
				classes[i] = classes[i].length() > 0 ? classes[i].substring(0, classes[i].length() - 1) : classes[i];
				outStreamWriter.append(classes[i]);
	...
	public static double[][] readCentroids(int clusters, int index, String dir, int dimensions) {
		...
		try {
			FileSystem hdfs = FileSystem.get(new Configuration());
			Path centroidFile = new Path(dir + "/centroid" + Integer.toString(index - 1));
	...
	public static class InitMapper extends Mapper<LongWritable, Text, IntWritable, TextArrayWritable> {
		...
		public void map(LongWritable id, Text value, Context context) throws IOException, InterruptedException {
			...
			String[] values = line.split(",");
			if (values != null && values.length == dimensions + 1) {
				context.write(new IntWritable(1 + random.nextInt(clusters)), new TextArrayWritable(values));
			}
		}
	}

	public static class InitReducer extends Reducer<IntWritable, TextArrayWritable, Text, Text> {
		...
		@Override
		public void reduce(IntWritable id, Iterable<TextArrayWritable> values, Context context)
			...
			for (TextArrayWritable value : values) {
				...
				lastPoint = data;
				context.write(new Text(id.toString() + ","), new Text(data + point[dimensions]));
			}
			for (int i = 0; i < center.length; i++) {
	...
	public static class IterationMapper extends Mapper<LongWritable, Text, IntWritable, TextArrayWritable> {
		...
		protected void setup(Context context) throws IOException, InterruptedException {
			...
			output = config.get("Output");
			centroid = readCentroids(clusters, iteration, output, Integer.valueOf(dimensions));
		}
		...
		public void map(LongWritable id, Text value, Context context) throws IOException, InterruptedException {
			...
			context.write(new IntWritable(clusterId),
					new TextArrayWritable(Arrays.copyOfRange(values, 1, values.length)));
		}
	}

	public static class IterationReducer extends Reducer<IntWritable, TextArrayWritable, Text, Text> {
		private int clusters;
		private int dimensions;
		private int iteration = 0;
		private String output = "";
	...
	public static boolean converge(int clusters, int index, String dir, int dimensions, int maxIterations)
		...
		for (int i = 0; i < clusters; i++) {
			...
			for (int j = 0; j < centroid1[i].length; j++) {
				euclidean += Math.pow(centroid1[i][j] - centroid2[i][j], 2);
			}
	...
	public static void main(String[] args) throws Exception {
		if (args.length > 5 || args.length < 4) {
			System.out.print(
					"args should be 4 or 5: <input path> <output path> <number of clusters> <dimensions> <max number of iterations>.");
			System.exit(-1);
		}
		String input = args[0];
		...
		FileOutputFormat.setOutputPath(initJob, new Path(output + "/iteration" + Integer.toString(iteration)));

		initJob.waitForCompletion(true);

		Configuration conf = new Configuration();
		while (!converge(Integer.parseInt(clusters), iteration, output, Integer.valueOf(dimensions),
			...
			Path jobInput = new Path(output + "/iteration" + Integer.toString(iteration - 1) + "/part-r-00000");
			Path jobOutput = new Path(output + "/iteration" + Integer.toString(iteration));
